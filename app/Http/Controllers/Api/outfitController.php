...
class outfitController extends Controller
{
    ...
    public function store(Request $request,Filesystem $filesystem)
    {
        ...
        if($request->get('date_day')){
            $book->date_day = $request->get('date_day');
        } if($request->get('car_type')){
        $book->car_type = $request->get('car_type');
    }
        if($request->get('number')){
            $book->number = $request->get('number');
        } if($request->get('date_hour')){
        $book->date_hour = $request->get('date_hour');
    }if($request->get('car_brand')){
        $book->car_brand = $request->get('car_brand');
    }
    ...
    public function editoutfit(Request $request, Filesystem $filesystem)
    {
        $v = Validator::make($request->all(), [
            'title' => 'required',
            'needtype' => 'required',
        ...
        $user->update();
        //book
        $book = outfit::where('adver_id', $advertise->id)->first();
        $book->destination = $request->get('destination');
        $book->gender = $request->get('gender');
        $book->date_day = $request->get('date_day');
        $book->car_type = $request->get('car_type');
